...
**慣習：** NumPyは `np` という短い名前でインポートするのが一般的です。
...
`ndarray`は「N-dimensional array（N次元配列）」の略です。
...
python_x = [1, 2, 3]
python_y = [4, 5, 6]
# python_x + python_y  # [1, 2, 3, 4, 5, 6] ← 連結になる！
# python_x * python_y  # エラー！
```
...
NumPyは多次元配列を扱えます。これがディープラーニングで非常に重要です。
...
print(A.ndim)   # 2 → 2次元
print(A.dtype)  # dtype('int64') → 要素のデータ型
```
...
**形状（shape）** は配列の次元を表す重要な属性です。ディープラーニングでは、常に形状を意識する必要があります。
...
**ブロードキャスト**は、形状が異なる配列同士の演算を可能にするNumPyの強力な機能です。
...
- コードがシンプルで読みやすくなる
- NumPyの内部最適化により高速
- バッチ処理（複数データを一度に処理）が簡単

**実例：画像の正規化**
```python
# 画像データ（0〜255の値）を0〜1に正規化
image = np.array([[100, 200], [150, 250]])
normalized = image / 255.0  # ブロードキャストで全要素を255で割る
print(normalized)
# [[0.39 0.78]
#  [0.59 0.98]]
```
